...
const SegmentedRadioGroup: React.FC<SegmentedRadioGroupProps> = ({ segmented = '', setSegmented, data = [], segmentedDom }) => {
      ...
      {isMobile ? (
        <Radio.Group optionType="button" buttonStyle="solid" onChange={(e: RadioChangeEvent) => setSegmented(e?.target?.value as SegmentedValue)} value={segmented} className={styles.radioGroup}>
          <Space style={{ width: '100%' }} direction={isMobile ? 'vertical' : 'horizontal'}>
            {data?.map((type) => (
              <Radio style={{ width: '100%' }} key={type} value={type}>
                {type}
              </Radio>
            ))}
          </Space>
        ...
        <Segmented ref={segmentedDom} style={{ backgroundColor: '#fff' }} block value={segmented} options={data} onChange={(value: SegmentedValue) => setSegmented(value)} />
      )}
    </div>
  );
};
